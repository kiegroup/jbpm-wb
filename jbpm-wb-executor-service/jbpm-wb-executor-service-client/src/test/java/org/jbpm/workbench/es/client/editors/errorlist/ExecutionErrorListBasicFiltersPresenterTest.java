...
public class ExecutionErrorListBasicFiltersPresenterTest extends AbstractBasicFiltersPresenterTest {
    ...
    public void onActiveFilterAddedTest() {
        ...
        when(activeFilterItemMock.getKey()).thenReturn(Constants.INSTANCE.Type());
        when(activeFilterItemMock.getValue()).thenReturn(Arrays.asList(Constants.INSTANCE.Task(), Constants.INSTANCE.DB()));
        presenter.onActiveFilterAdded(activeFilterItemMock);
        verify(getView()).checkSelectFilter(Constants.INSTANCE.Type(), Constants.INSTANCE.Task());
        verify(getView()).checkSelectFilter(Constants.INSTANCE.Type(), Constants.INSTANCE.DB());
    }
}
