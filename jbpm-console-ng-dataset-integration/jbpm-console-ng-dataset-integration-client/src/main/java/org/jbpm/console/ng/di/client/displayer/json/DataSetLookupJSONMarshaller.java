 ...
 */
package org.jbpm.console.ng.di.client.displayer.json;
...
public class DataSetLookupJSONMarshaller {
    ...
    private JSONObject formatColumnFilter( ColumnFilter columnFilter ) {
        ...
        if ( columnFilter instanceof LogicalExprFilter ) {
            ...
            GWT.log( TableDisplayerConstants.INSTANCE.json_datasetlookup_unsupported_column_filter() );
        }
    ...
    private ColumnFilter parseColumnFilter( JSONObject columnFilterJson ) {
        ...
        if ( checkNotNull( value, false, TableDisplayerConstants.INSTANCE.json_datasetlookup_columnfilter_null_columnid() ) ) {
            columnId = value.isString() != null ? value.isString().stringValue() : null;
        }

        value = columnFilterJson.get( FUNCTION_TYPE );
        if ( checkNotNull( value, false, TableDisplayerConstants.INSTANCE.json_datasetlookup_columnfilter_null_functiontype() ) ) {
        ...
        if ( isCoreFilter( functionType ) ) {
            ...
            if ( checkNotNull( value, false, TableDisplayerConstants.INSTANCE.json_datasetlookup_corefunction_null_params() ) ) {
            ...
            if ( checkNotNull( value, false, TableDisplayerConstants.INSTANCE.json_datasetlookup_logexpr_null_params() ) ) {
            ...
            throw new RuntimeException( TableDisplayerConstants.INSTANCE.json_datasetlookup_columnfilter_wrong_type() );
    }
    ...
    private boolean checkNotNull( JSONValue value, boolean nullable, String errorMessage ) {
        if ( nullable ) return value != null;
            ...
            if ( value != null ) return true;
                ...
                throw new RuntimeException( TableDisplayerConstants.INSTANCE.json_datasetlookup_validation_error() + ( !StringUtils.isBlank( errorMessage ) ? errorMessage : "" ) );
        }
    }
}
