...
public class TaskServiceEntryPointImpl implements TaskServiceEntryPoint {
    ...
    public Map<Day, List<TaskSummary>> getTasksOwnedFromDateToDateByDays(String userId, List<String> strStatuses,
            Date javaUtilDateFrom, Date javaUtilDateTo, String language) {
    ...
    public Map<Day, List<TaskSummary>> getTasksAssignedAsPotentialOwnerFromDateToDateByDays(String userId,
            List<String> strStatuses, Date from, Date to, String language) {
        List<Status> statuses = new ArrayList<Status>();
        for (String s : strStatuses) {
            statuses.add(Status.valueOf(s));
        }
        Map<Day, List<TaskSummary>> tasksByDay = new LinkedHashMap<Day, List<TaskSummary>>();
        List<TaskSummary> firstDayTasks = TaskSummaryHelper.adaptCollection(
                taskService.getTasksAssignedAsPotentialOwnerByExpirationDateOptional(
                        userId, statuses, from));
        //This is a hack we need to find a way to get the PotentialOwners in a performant way
        ...
        for (int i = 1; i <= nrOfDays; i++) {
            ...
            List<TaskSummary> dayTasks = TaskSummaryHelper.adaptCollection(
                    taskService.getTasksAssignedAsPotentialOwnerByExpirationDate(userId,
                            statuses, currentDay));
            //This is a hack we need to find a way to get the PotentialOwners in a performant way
    ...
    @Override
    public Map<Day, List<TaskSummary>> getTasksAssignedAsPotentialOwnerFromDateToDateByDays(String userId, Date from,
            int nrOfDaysTotal, String language) {
        long plusDays = (nrOfDaysTotal - 1) * (long) DateTimeConstants.MILLIS_PER_DAY;
        return getTasksAssignedAsPotentialOwnerFromDateToDateByDays(userId, from, new Date(from.getTime() + plusDays),
                language);
    }
    ...
    public Map<Day, List<TaskSummary>> getTasksAssignedAsPotentialOwnerFromDateToDateByDays(String userId, Date
            from, Date to, String language) {
        List<String> statuses = new ArrayList<String>();
        statuses.add("Ready");
        statuses.add("InProgress");
        statuses.add("Reserved");
    ...
    @Override
    public Map<Day, List<TaskSummary>> getTasksAssignedFromDateToDateByGroupsByDays(String userId, List<String> groupIds,
            Date from, int nrOfDaysTotal, String language) {
        long plusDays = (nrOfDaysTotal - 1) * (long) DateTimeConstants.MILLIS_PER_DAY;
        return getTasksAssignedFromDateToDateByGroupsByDays(userId, groupIds, from, new Date(from.getTime() + plusDays),
                language);
    }
