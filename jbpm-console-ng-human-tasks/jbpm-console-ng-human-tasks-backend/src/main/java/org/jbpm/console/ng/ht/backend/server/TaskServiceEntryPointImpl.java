...
<<<<<<< HEAD
	...
	public List<TaskSummary> getTasksAssignedAsPotentialOwnerByExpirationDateOptional(
			String userId, List<String> status, Date from, String language) { // @TODO:
																				// MUST
																				// ADD
																				// LANGUAGE
																				// FILTER
		...
		List<TaskSummary> taskSummaries = TaskSummaryHelper
				.adaptCollection(taskService
						.getTasksAssignedAsPotentialOwnerByExpirationDateOptional(
								userId, statuses, from));
		// This is a hack we need to find a way to get the PotentialOwners in a
		// performant way
	...
	public List<TaskSummary> getTasksOwnedByExpirationDateOptional(
		...
		List<TaskSummary> taskSummaries = TaskSummaryHelper
				.adaptCollection(taskService
						.getTasksOwnedByExpirationDateOptional(userId,
								statuses, from));
		// This is a hack we need to find a way to get the PotentialOwners in a
		// performant way
		List<Long> taskIds = new ArrayList<Long>(taskSummaries.size());
	...
	public Map<Day, List<TaskSummary>> getTasksAssignedFromDateToDateByDays(
			String userId, Date from, Date to, String language) {
		return getTasksOwnedFromDateToDateByDays(userId, from, to, language);
	}

	public Map<Day, List<TaskSummary>> getTasksOwnedFromDateToDateByDays(
			String userId, List<String> strStatuses, Date from, Date to,
			String language) {
		List<Status> statuses = new ArrayList<Status>();
		...
		List<TaskSummary> firstDayTasks = TaskSummaryHelper
				.adaptCollection(taskService
						.getTasksOwnedByExpirationDateOptional(userId,
								statuses, from));
		// This is a hack we need to find a way to get the PotentialOwners in a
		...
		int nrOfDays = Days.daysBetween(new LocalDate(from), new LocalDate(to))
				.getDays();
		for (int i = 1; i <= nrOfDays; i++) {
			...
			List<TaskSummary> dayTasks = TaskSummaryHelper
					.adaptCollection(taskService.getTasksOwnedByExpirationDate(
							userId, statuses, currentDay));
			// This is a hack we need to find a way to get the PotentialOwners
			// in a performant way
			...
			if (taskIds.size() > 0) {
				potentialOwnersForTaskIds = getPotentialOwnersForTaskIds(taskIds);
				for (TaskSummary ts : dayTasks) {
					ts.setPotentialOwners(potentialOwnersForTaskIds.get(ts
							.getId()));
				}
			}
			tasksByDay.put(new Day(currentDay, dayFormat.format(currentDay)),
					dayTasks);
		}
		return tasksByDay;
	}
	...
	@Override
	public Map<Day, List<TaskSummary>> getTasksAssignedAsPotentialOwnerFromDateToDateByDays(
			String userId, List<String> strStatuses, Date from,
			int nrOfDaysTotal, String language) {
		long plusDays = (nrOfDaysTotal - 1)
				* (long) DateTimeConstants.MILLIS_PER_DAY;
		return getTasksAssignedAsPotentialOwnerFromDateToDateByDays(userId,
				strStatuses, from, new Date(from.getTime() + plusDays),
				language);
	}
	...
	@Override
	public Map<Day, List<TaskSummary>> getTasksAssignedAsPotentialOwnerFromDateToDateByDays(
			String userId, Date from, int nrOfDaysTotal, String language) {
		long plusDays = (nrOfDaysTotal - 1)
				* (long) DateTimeConstants.MILLIS_PER_DAY;
		return getTasksAssignedAsPotentialOwnerFromDateToDateByDays(userId,
				from, new Date(from.getTime() + plusDays), language);
	}
	...
	public Map<Day, List<TaskSummary>> getTasksOwnedFromDateToDateByDays(
		...
		long plusDays = (nrOfDaysTotal - 1)
				* (long) DateTimeConstants.MILLIS_PER_DAY;
		return getTasksOwnedFromDateToDateByDays(userId, strStatuses, from,
	...
	public List<TaskSummary> getTasksAssignedByGroup(String userId,
			String groupId, String language) {
		List<org.kie.api.task.model.TaskSummary> tasksAssignedAsPotentialOwner = taskService
				.getTasksAssignedAsPotentialOwner(userId, language);
	...
	@Override
	public Map<Day, List<TaskSummary>> getTasksAssignedFromDateToDateByGroupsByDays(
			String userId, List<String> groupIds, Date from, int nrOfDaysTotal,
			String language) {
		long plusDays = (nrOfDaysTotal - 1)
				* (long) DateTimeConstants.MILLIS_PER_DAY;
		return getTasksAssignedFromDateToDateByGroupsByDays(userId, groupIds,
				from, new Date(from.getTime() + plusDays), language);
	}

	public Map<Day, List<TaskSummary>> getTasksAssignedFromDateToDateByGroupsByDays(
			String userId, List<String> groupIds, Date from, Date to,
			String language) {
		...
		for (int i = 1; i <= nrOfDays; i++) {
			long plusDays = i * (long) DateTimeConstants.MILLIS_PER_DAY;
			Date currentDay = new Date(from.getTime() + plusDays);
			List<TaskSummary> currentDayGroupTasks = TaskSummaryHelper
					.adaptCollection(taskService
							.getTasksAssignedAsPotentialOwnerByExpirationDate(
									userId, statuses, currentDay));
			// This is a hack we need to find a way to get the PotentialOwners
	...
	@Override
	public long addTask(String taskString, Map<String, Object> inputs,
	...
	@Override
	public long addTaskAndStart(String taskString, Map<String, Object> inputs,
			String userId, Map<String, Object> templateVars) {
	...
	public void setTaskNames(long taskId, List<String> taskNames) {
		taskService.setTaskNames(taskId,
				TaskI18NHelper.adaptI18NList(taskNames));
	}
	...
	public void setDescriptions(long taskId, List<String> descriptions) {
		taskService.setDescriptions(taskId,
				TaskI18NHelper.adaptI18NList(descriptions));
	}
	...
	public void setSubTaskStrategy(long taskId, String strategy) {
		taskService.setSubTaskStrategy(taskId,
				SubTasksStrategy.valueOf(strategy));
	}
	...
	public List<String> getDescriptions(long taskId) {
		return TaskI18NHelper.adaptStringList(taskService
	...
	public TaskSummary getTaskDetails(long taskId) {
		...
		List<OrganizationalEntity> potentialOwners = task
		...
		return new TaskSummary(
				...
				((task.getNames() != null && task.getNames().size() > 0)) ? task
						.getNames().get(0).getText()
						: "",
				((task.getSubjects() != null && task.getSubjects().size() > 0)) ? task
						.getSubjects().get(0).getText()
						: "", ((task.getDescriptions() != null && task
						.getDescriptions().size() > 0)) ? task
						.getDescriptions().get(0).getText() : "", task
						.getTaskData().getStatus().name(), task.getPriority(),
				task.getTaskData().isSkipable(), (task.getTaskData()
						.getActualOwner() != null) ? task.getTaskData()
						.getActualOwner().getId() : "", (task.getTaskData()
						.getCreatedBy() != null) ? task.getTaskData()
						.getCreatedBy().getId() : "", task.getTaskData()
						.getCreatedOn(),
				task.getTaskData().getActivationTime(), task.getTaskData()
						.getExpirationTime(),
				task.getTaskData().getProcessId(), task.getTaskData()
						.getProcessSessionId(), ((InternalTask) task)
						.getSubTaskStrategy().name(), (int) task.getTaskData()
						.getParentId(), potOwnersString);
	}
	...
	public Map<String, String> getContentListById(long contentId) {
		...
		Object unmarshall = ContentMarshallerHelper.unmarshall(
				contentById.getContent(), null);
	...
	public Map<String, String> getContentListByTaskId(long taskId) {
		...
		long documentContentId = taskInstanceById.getTaskData()
	...
	public IdentitySummary getOrganizationalEntityById(String entityId) {
		OrganizationalEntity entity = taskService
	...
	@Override
	public long addComment(long taskId, String text, String addedBy,
			Date addedOn) {
	...
	public List<CommentSummary> getAllCommentsByTaskId(long taskId) {
		return CommentSummaryHelper.adaptCollection(taskService
				.getAllCommentsByTaskId(taskId));
	}

	@Override
	public CommentSummary getCommentById(long commentId) {
		return CommentSummaryHelper
				.adapt(taskService.getCommentById(commentId));
	}

	@Override
	public void updateSimpleTaskDetails(long taskId, List<String> taskNames,
			int priority, List<String> taskDescription,
	...
	@Override
	public void completeBatch(List<Long> taskIds, String user,
...
>>>>>>> 66b13c70c6c5ee3b6bbd0eae6eb38648d1936ca9
}
