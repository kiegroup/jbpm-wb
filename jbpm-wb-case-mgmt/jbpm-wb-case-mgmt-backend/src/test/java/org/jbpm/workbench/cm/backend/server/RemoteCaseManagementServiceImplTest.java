...
public class RemoteCaseManagementServiceImplTest {
    ...
    public void getCaseInstances_bulkCaseInstances() {
        ...
        List<CaseInstance> expectedFirstPage = caseInstances.subList(0,
                                                                     20);
        List<CaseInstance> expectedSecondPage = caseInstances.subList(20,
                                                                      40);
        List<CaseInstance> expectedThirdPage = caseInstances.subList(40,
                                                                     55);

        when(clientMock.getCaseInstances(singletonList(request.getStatus().getName()),
                                         0,
                                         pageSize)).thenReturn(expectedFirstPage);
        List<CaseInstanceSummary> actualFirstPage = testedService.getCaseInstances(request,
                                                                                   0,
                                                                                   pageSize);
        assertEquals(20,
                     actualFirstPage.size());

        when(clientMock.getCaseInstances(singletonList(request.getStatus().getName()),
                                         1,
                                         pageSize)).thenReturn(expectedSecondPage);
        List<CaseInstanceSummary> actualSecondPage = testedService.getCaseInstances(request,
                                                                                    1,
                                                                                    pageSize);
        assertEquals(20,
                     actualSecondPage.size());

        when(clientMock.getCaseInstances(singletonList(request.getStatus().getName()),
                                         2,
                                         pageSize)).thenReturn(expectedThirdPage);
        List<CaseInstanceSummary> actualThirdPage = testedService.getCaseInstances(request,
                                                                                   2,
                                                                                   pageSize);
        assertEquals(15,
                     actualThirdPage.size());
    }
    ...
    public void testGetComments_bulkComments() {
        ...
        List<CaseComment> expectedFirstPage = caseComments.subList(0,
                                                                   20);
        List<CaseComment> expectedSecondPage = caseComments.subList(20,
                                                                    40);
        List<CaseComment> expectedThirdPage = caseComments.subList(40,
                                                                   55);
    ...
    public void getCaseMilestones_bulkMilestones() {
        ...
        for (int i = 0; i < 55; i++) {
            caseMilestones.add(createTestMilestone("id" + i,
                                                   "milestone" + i,
                                                   "Available"));
        }

        List<CaseMilestone> expectedFirstPage = caseMilestones.subList(0,
                                                                       20);
        List<CaseMilestone> expectedSecondPage = caseMilestones.subList(20,
                                                                        40);
        List<CaseMilestone> expectedThirdPage = caseMilestones.subList(40,
                                                                       55);

        when(clientMock.getMilestones(containerId,
                                      caseId,
                                      false,
                                      0,
                                      pageSize)).thenReturn(expectedFirstPage);
        List<CaseMilestoneSummary> actualFirstPage = testedService.getCaseMilestones(containerId,
                                                                                     caseId,
                                                                                     new CaseMilestoneSearchRequest(),
                                                                                     0,
                                                                                     pageSize);
        assertEquals(expectedFirstPage.size(),
                     actualFirstPage.size());
        ...
        when(clientMock.getMilestones(containerId,
                                      ...
                                      1,
                                      pageSize)).thenReturn(expectedSecondPage);
        List<CaseMilestoneSummary> actualSecondPage = testedService.getCaseMilestones(containerId,
                                                                                      caseId,
                                                                                      new CaseMilestoneSearchRequest(),
                                                                                      1,
                                                                                      pageSize);
        assertEquals(expectedSecondPage.size(),
                     actualSecondPage.size());
        ...
        when(clientMock.getMilestones(containerId,
                                      ...
                                      2,
                                      pageSize)).thenReturn(expectedThirdPage);
        List<CaseMilestoneSummary> actualThirdPage = testedService.getCaseMilestones(containerId,
                                                                                     caseId,
                                                                                     new CaseMilestoneSearchRequest(),
                                                                                     2,
                                                                                     pageSize);
        assertEquals(expectedThirdPage.size(),
                     actualThirdPage.size());
    }
}