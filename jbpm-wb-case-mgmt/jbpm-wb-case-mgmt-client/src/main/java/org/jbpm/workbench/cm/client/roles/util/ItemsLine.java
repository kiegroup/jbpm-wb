...
public class ItemsLine extends AbstractView<CaseRolesPresenter> {
    ...
    public void initWithSingleItem(String itemsLineId,
                                   String item) {
    ...
    public void initWithItemsLine(int maxWidth,
                                  String itemsLineId,
            ...
            for (CaseRolesPresenter.CaseAssignmentItem item : items) {
                if (!overflow) {
                    ...
                    if (getVisibleUnorderedList().getBoundingClientRect().getWidth().intValue() + MORE_LINK_SIZE > maxWidth) {
                        ...
                        removeCSSClass(getMoreItemsLinkDiv(),
                                       "hidden");
                    }
                } else {
                    addItem(item,
    ...
    public void showAllItems() {
        removeCSSClass(customPopover,
                       "hidden");
        popoverOpen = true;
    }

    public void hideAllItems() {
        addCSSClass(customPopover,
                    "hidden");
        popoverOpen = false;
    }

    public void addItem(final CaseRolesPresenter.CaseAssignmentItem item,
                        UnorderedList itemList) {
        final HTMLElement closeIcon = createElement("span");
        addCSSClass(closeIcon,
                    "pficon");
        addCSSClass(closeIcon,
                    "pficon-close");
        addCSSClass(closeIcon,
