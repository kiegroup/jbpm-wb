...
@Dependent
public class WorkbranchConfigureationPopup extends BaseModal {
    ...
    interface AddNewKBasePopupViewImplBinder extends UiBinder<Widget, WorkbranchConfigureationPopup> {

    }

    public WorkbranchConfigureationPopup() {
        ...
        setMutipleModeItems(getAllMutipleModes());
    ...
    public void onOk(){
   	 ...
   	 final Node<String, String> selectedMutipleMode = getSelectedMutipleModeItem();
   	 boolean refrashPerspectiveFlag = true;
   	 boolean refrashWorkbranchFlag = true;
   	 if (selectedLanguageItem.getValue().equals(LocaleInfo.getCurrentLocale().getLocaleName())){
   		 refrashWorkbranchFlag = false;
   	 }
   	 if (selectedMutipleMode.getValue().equals(contextualSwtichMode.getModeName())){
  ...
  **/
    ...
    private void setLanguageListItems(List<Node<String, String>> items) {
		 languageListItems.clear();
	        for ( Node<String, String> item : items ) {
	            languageListItems.addItem( item.getKey(),
	                               item.getValue() );
	        }
	}
    ...
    private List<Node<String, String>> getAllSupportLanguage(){
    	...
    	List<Node<String, String>> allSupportLanguage = new ArrayList<Node<String, String>>(languages.length);
    	for(String language:languages){
    		Node<String, String> entry = new Node<String, String>(languageMap.get(language), language);
    ...
    /**
     * Mutiple mode function
     **/
	private void refrashWorkbranch(
			final Node<String, String> selectedLanguageItem ,boolean refrashWorkbranchFlag) {
		if(refrashWorkbranchFlag == false){
			return;
		}
		if ( selectedLanguageItem != null && !selectedLanguageItem.getValue().isEmpty() ) {
    ...
    private List<Node<String, String>> getAllMutipleModes(){
    	List<Node<String, String>> allMutipleMode = new ArrayList<Node<String, String>>(2);
    	allMutipleMode.add(new Node<String, String>( constants.Advanced(),contextualSwtichMode.ADVANCED_MODE ));
    	allMutipleMode.add(new Node<String, String>( constants.Basic(),contextualSwtichMode.BASIC_MODE ));
    	return allMutipleMode;
    	
    } 
    
    private void setMutipleModeItems(List<Node<String, String>> items) {
    	mutipleModeItems.clear();
	        for ( Node<String, String> item : items ) {
	        	mutipleModeItems.addItem( item.getKey(),
	                               item.getValue() );
	        }
	        
	}
    
    private Node<String, String> getSelectedMutipleModeItem() {
		final int selectedIndex = mutipleModeItems.getSelectedIndex();
        if ( selectedIndex == -1 ) {
            return new Node<String, String>( "", "" );
        }
        final String text = mutipleModeItems.getItemText( selectedIndex );
        final String value = mutipleModeItems.getValue( selectedIndex );
        return new Node<String, String>( text, value );
	}
    
    private void switchMode(boolean refrashPerspectiveFlag,boolean refrashWorkbranchFlag){
		if (refrashPerspectiveFlag && !refrashWorkbranchFlag){
			...
			if (modeName.equals(BASIC_MODE)) {
				contextualSwtichMode.setModeName(ADVANCED_MODE);
			} else {
				contextualSwtichMode.setModeName(BASIC_MODE);
			}
			refrashPerspective();
		}
    }
