...
public class CaseActionsPresenterTest extends AbstractCaseInstancePresenterTest {

    @Mock(name = "view")
    CaseActionsPresenter.CaseActionListView caseActionsView;

    @Mock
    CaseActionsPresenter.CaseSimpleActionListView caseSimpleActionListView;

    CaseActionSearchRequest caseActionsSearchRequest;
    ...
    public void init() {
        ...
        caseActionsSearchRequest = new CaseActionSearchRequest();
        when(caseActionsView.getCaseActionSearchRequest()).thenReturn(caseActionsSearchRequest);
        ...
        when(caseManagementService.getCaseActions(anyString(), anyString(), any(CaseActionSearchRequest.class),anyString())).thenReturn(caseActionSummaryList);
    }

    @Test
    public void testLoadCaseInstance() {
        caseActionsSearchRequest.setFilterBy(CaseActionsFilterBy.AVAILABLE);
        presenter.setSimpleActionListView(caseSimpleActionListView);
        setupCaseInstance(cis, serverTemplateId);

        verify(caseSimpleActionListView).setCaseActionList(caseActionSummaryList);
        verify(caseSimpleActionListView).updateListHeader(null,caseActionsSearchRequest.getFilterBy());
    }
